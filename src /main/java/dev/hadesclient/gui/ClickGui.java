...
public final class ClickGui extends UiScreen {
        ...
        for (int i = 0; i < TABS.length; i++) {
            ...
            Element pill = new Element() {
                @Override protected void paint(Ctx ctx, DrawContext g) {
                    ...
                    Draw.roundRect(g, x, y, w, h, 3f, fill);
                    Draw.roundOutline(g, x, y, w, h, 3f, 1f, edge);
    ...
    private Element sidebarItem(String label, boolean active, Runnable onClick) {
        return new Element() {
            @Override protected void paint(Ctx ctx, DrawContext g) {
                ...
                float lift = hover.get();
                if (active) {
                    Draw.roundRect(g, x, y, w, h, 2f, theme.accent().alpha(0.15f));
                    Draw.roundRect(g, x, y + 4, 2f, h - 8, 1f, theme.accent());
                } else if (lift > 0.01f) {
                    Draw.roundRect(g, x, y, w, h, 2f, theme.raised().alpha(0.25f * lift));
                }
    ...
    private void buildWidgetSettings() {
        ...
        Element header = new Element() {
            @Override protected void paint(Ctx ctx, DrawContext g) {
                Theme theme = ctx.theme();
                Draw.roundRect(g, x, this.y, w, h, 3f, theme.raised().alpha(0.35f));
                Draw.roundOutline(g, x, this.y, w, h, 3f, 1f, theme.stroke().alpha(0.4f));
                ...
                Draw.roundRect(g, x + w - bw - 10, this.y + 10, bw, 16, 2f, theme.accent().alpha(0.15f));
    ...
    private float buildStringListEditor(float rx, float ry, float rw, Setting.StringList list) {
        ...
        for (int i = 0; i < list.get().size(); i++) {
            ...
            Element entryRow = new Element() {
                @Override protected void paint(Ctx ctx, DrawContext g) {
                    Theme theme = ctx.theme();
                    Draw.roundRect(g, x, y, w, h, 2f, theme.raised().alpha(0.15f + 0.15f * hover.get()));
                    Draw.textInRow(g, name, x + 12, y, h, theme.text());
                    // Red X button
                    float bx = x + w - 22f;
                    float by = y + (h - 12f) / 2f;
                    Draw.roundRect(g, bx, by, 16f, 12f, 2f, theme.bad().alpha(0.2f + 0.3f * hover.get()));
        ...
        for (Theme theme : HadesClient.themes().all()) {
            ...
            Element row = new Element() {
                @Override protected void paint(Ctx ctx, DrawContext g) {
                    ...
                    float lift = hover.get();
                    Draw.roundRect(g, x, this.y, w, h, 2f, active.raised().alpha(0.25f + 0.3f * lift));
                    Draw.roundOutline(g, x, this.y, w, h, 2f, 1f,
                    ...
                    for (Color swatch : List.of(value.accent(), value.panel(), value.raised(), value.ok(), value.bad())) {
                        Draw.roundRect(g, sx, this.y + h / 2f - 6, 12, 12, 1f, swatch);
                        sx += 15;
                    }
    ...
    private Element settingRow(float rx, float ry, float rw, String label) {
        Element row = new Element() {
            @Override protected void paint(Ctx ctx, DrawContext g) {
                Theme theme = ctx.theme();
                Draw.roundRect(g, x, y, w, h, 2f, theme.raised().alpha(0.15f + 0.1f * hover.get()));
        ...
        for (int i = 0; i < cards.size(); i++) {
            ...
            Element element = new Element() {
                ...
                @Override protected void paint(Ctx ctx, DrawContext g) {
                    ...
                    float lift = hover.get();
                    float active = on.get();
                    Draw.roundRect(g, x, y, w, h, 3f, theme.raised().alpha(0.30f + 0.25f * lift));
                    Draw.roundOutline(g, x, y, w, h, 3f, 1f,
                    ...
                    Draw.roundRect(g, x + 8, pillY, w - 16, 20f, 3f, pill);
    ...
    protected void backdrop(Ctx ctx, DrawContext g) {
        ...
        Draw.roundRect(g, winX, winY, winW, winH, 3f, theme.panel().alpha(0.35f * in));
        Draw.roundOutline(g, winX, winY, winW, winH, 3f, 1f, theme.stroke().alpha(0.5f * in));
