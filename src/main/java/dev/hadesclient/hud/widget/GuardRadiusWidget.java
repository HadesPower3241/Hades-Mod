...
public final class GuardRadiusWidget extends HudWidget {
    ...
    public void render(DrawContext g, Theme theme, float x, float y) {
        ...
        if (nearestName == null) {
            size(WIDTH, 16f);
            if (showBg()) Draw.roundRect(g, x, y, WIDTH, 16f, 3f, theme.panel().alpha(bgAlpha() * 0.4f));
            Draw.text(g, "No guards nearby", x + 6f, y + 3f, theme.faint());
            return;
        }
        ...
        if (showBg()) {
            ...
            Draw.roundOutline(g, x, y, WIDTH, h, 3f, 1f, edgeColor.alpha(0.6f));
        }

        // Guard name + distance
        String label = Draw.fit(nearestName, WIDTH - 60f);
        Draw.text(g, label, x + 6f, y + 3f, theme.text());
        String dist = String.format(Locale.ROOT, "%.0fm", nearestDist);
        Draw.text(g, dist, x + WIDTH - Draw.textWidth(dist) - 6f, y + 3f, edgeColor);
        ...
        if (showBar.get()) {
            ...
            Draw.text(g, status, x + 6f, y + 26f, edgeColor);
        }
    }
}
