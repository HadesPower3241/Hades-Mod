...
public final class DirectionWidget extends HudWidget {
    private final Setting.ColorVal cardinalColor = setting(new Setting.ColorVal("cardinalColor", "Cardinal Color", 0x55FFFF));
    ...
    @Override public String description() { return "360° scrolling compass showing your facing direction."; }

    @Override
    public void render(DrawContext g, Theme theme, float x, float y) {
        MinecraftClient client = mc();
        if (client == null || client.player == null) {
            if (!isEditor()) { size(WIDTH, HEIGHT); return; }
        }
        ...
        float yaw = (client != null && client.player != null) ? client.player.getYaw() : 45f;
        // Normalize to 0-360
        yaw = ((yaw % 360f) + 360f) % 360f;

        chrome(g, x, y, WIDTH, HEIGHT, 0f);
