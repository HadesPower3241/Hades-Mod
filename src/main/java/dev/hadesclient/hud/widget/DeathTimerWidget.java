...
public final class DeathTimerWidget extends HudWidget {
    ...
    @Override
    public String description() { return enabled() ? "Stronghold / Outpost lockouts" : "Hidden"; }
    ...
    public void render(DrawContext g, Theme theme, float x, float y) {
        ...
        float ry = y;
        if (header) {
            Draw.roundRect(g, x, ry, WIDTH, ROW_H, 4f, theme.raised().alpha(0.85f));
            Draw.text(g, "Death Timers", x + PAD, ry + (ROW_H - Draw.textHeight()) / 2f,
    ...
    private void drawRow(DrawContext g, Theme theme, float x, float y, ActiveTimer t) {
        ...
        Draw.roundRect(g, x, y, WIDTH, ROW_H, 4f, bg);
        Draw.roundOutline(g, x, y, WIDTH, ROW_H, 4f, 1f, edge.alpha(0.55f));

        String label = Draw.fit(t.displayName(), WIDTH - PAD * 2 - 50f);
        Draw.text(g, label, x + PAD, y + (ROW_H - Draw.textHeight()) / 2f, theme.text());

        String time = formatTime(t.remainingSeconds());
        float tw = Draw.textWidth(time);
        Draw.text(g, time, x + WIDTH - tw - PAD,
