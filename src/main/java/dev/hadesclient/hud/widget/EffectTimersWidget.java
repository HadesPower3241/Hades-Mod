...
public final class EffectTimersWidget extends HudWidget {
    ...
    public void render(DrawContext g, Theme theme, float x, float y) {
        ...
        if (background.get()) {
            Draw.roundRect(g, x, y, w, h, 6f, theme.panel().alpha(0.78f));
            Draw.roundOutline(g, x, y, w, h, 6f, 1f, theme.stroke().alpha(0.65f));
        }
        ...
        for (StatusEffectInstance effect : effects) {
            ...
            if (showNames.get()) {
                Draw.text(g, nameOf(effect), textX, rowY + 2f, theme.text());
                Draw.text(g, time, textX, rowY + 11f, timeColour);
            } else {
