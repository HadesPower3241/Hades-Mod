...
public final class JoinLeaveWidget extends HudWidget {
    private final Setting.ColorVal joinColor = setting(new Setting.ColorVal("joinColor", "Join Color", 0x55FF55));
    private final Setting.ColorVal leaveColor = setting(new Setting.ColorVal("leaveColor", "Leave Color", 0xFF5555));
    ...
    private static final float TOAST_H = 15f;
    private static final float TOAST_GAP = 2f;
    ...
    private final Setting.StringList whitelistPlayers = setting(
            new Setting.StringList("whitelistPlayers", "Whitelisted Players"));
    ...
    public void notifyEvent(String name, boolean join) {
        ...
        // Whitelist override: always allow whitelisted players
        if (!whitelistPlayers.get().isEmpty() && whitelistPlayers.contains(name)) { /* allowed */ }
        else if (ignoredPlayers.contains(name)) return;
    ...
    public void render(DrawContext g, Theme theme, float x, float y) {
        ...
        if (toasts.isEmpty()) {
            if (isEditor()) {
                // Show preview toasts
                float pw = 140f;
                size(pw, TOAST_H * 3 + TOAST_GAP * 2);
                chrome(g, x, y, pw, TOAST_H, 0f);
                Draw.rect(g, x + 2, y + 3, 3f, TOAST_H - 6, theme.ok());
                txt(g, "+ TellusPlayer", x + 9, y + (TOAST_H - Draw.textHeight()) / 2f, theme.text());
                float y2 = y + TOAST_H + TOAST_GAP;
                chrome(g, x, y2, pw, TOAST_H, 0f);
                Draw.rect(g, x + 2, y2 + 3, 3f, TOAST_H - 6, theme.bad());
                txt(g, "- JohnDoe", x + 9, y2 + (TOAST_H - Draw.textHeight()) / 2f, theme.text());
                float y3 = y2 + TOAST_H + TOAST_GAP;
                chrome(g, x, y3, pw, TOAST_H, 0f);
                Draw.rect(g, x + 2, y3 + 3, 3f, TOAST_H - 6, theme.ok());
                txt(g, "+ SkyWatcher", x + 9, y3 + (TOAST_H - Draw.textHeight()) / 2f, theme.text());
                return;
            }
            size(130f, TOAST_H); return;
        }
        ...
        for (Toast toast : toasts) {
            ...
            Draw.roundRect(g, tx, rowY, widest, TOAST_H, 0f, theme.panel().alpha(0.88f * eased));
            Draw.roundOutline(g, tx, rowY, widest, TOAST_H, 0f, 1f, accent.alpha(0.6f * eased));
            Draw.roundRect(g, tx + 2, rowY + 3, 3f, TOAST_H - 6, 0f, accent.alpha(eased));
    ...
    /**
     * Smart ignore matching:
     * - If pattern ends with "_" → prefix match only (warden_ matches warden_A3F2 but NOT wardentroller)
     * - Otherwise → substring match (giftshrine matches giftshrine1, giftshrine7, etc.)
     * - Always case-insensitive
     */
    private boolean matchesIgnoreList(String name) {
        if (name == null) return false;
        String lower = name.toLowerCase(java.util.Locale.ROOT);
        for (String pattern : ignoredPlayers.get()) {
            String p = pattern.toLowerCase(java.util.Locale.ROOT);
            if (p.endsWith("_")) {
                // Prefix match: "warden_" matches "warden_a3f2" but not "wardentroller"
                if (lower.startsWith(p)) return true;
            } else {
                // Substring match: "giftshrine" matches "giftshrine6", "giftshrine7"
