...
public final class JoinLeaveWidget extends HudWidget {
    ...
    private final Setting.Bool useIgnoreList = setting(
            new Setting.Bool("ignoreList", "Use ignore list", true));
    ...
    public void notifyEvent(String name, boolean join) {
        ...
        if (useIgnoreList.get() && ignored().contains(name)) return;    // exact-match ignore
        addToast(name, join);
    }

    // ---- Exact-name ignore list ------------------------------------------
    // Stored in config/hadesclient/joinleave-ignore.json as {"ignored":["bot1","bot2"]}.
    // Loaded lazily and cached in-memory. Edit the JSON while the game is running
    // and toggle the "Use ignore list" setting off/on to reload.
    private Set<String> ignoredCache;
    private long ignoredMtime;

    private Set<String> ignored() {
        try {
            java.nio.file.Path path = net.fabricmc.loader.api.FabricLoader.getInstance()
                    .getConfigDir().resolve("hadesclient").resolve("joinleave-ignore.json");
            if (!java.nio.file.Files.exists(path)) {
                java.nio.file.Files.createDirectories(path.getParent());
                java.nio.file.Files.writeString(path,
                        "{\n  \"_comment\": \"Exact player names to ignore in join/leave toasts. Case-sensitive; no substring matching. File is reloaded automatically after each edit.\",\n  \"ignored\": [\"warden1232142\", \"examplebot\"]\n}\n");
            }
            long mtime = java.nio.file.Files.getLastModifiedTime(path).toMillis();
            if (ignoredCache != null && mtime == ignoredMtime) return ignoredCache;
            Set<String> fresh = new HashSet<>();
            try (java.io.Reader r = java.nio.file.Files.newBufferedReader(path)) {
                com.google.gson.JsonElement root = com.google.gson.JsonParser.parseReader(r);
                if (root.isJsonObject()) {
                    com.google.gson.JsonElement list = root.getAsJsonObject().get("ignored");
                    if (list != null && list.isJsonArray()) {
                        for (com.google.gson.JsonElement e : list.getAsJsonArray()) {
                            if (e.isJsonPrimitive()) fresh.add(e.getAsString());
                        }
                    }
                }
            }
            ...
            if (ignoredCache == null) ignoredCache = new HashSet<>();
            return ignoredCache;
        }
    }
    ...
    @Override
    public HudCategory category() { return HudCategory.SOCIAL; }
