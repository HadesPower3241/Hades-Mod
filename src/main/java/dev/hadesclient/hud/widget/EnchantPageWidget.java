...
public final class EnchantPageWidget extends HudWidget {
    ...
    private final Setting.Bool showInventory = setting(new Setting.Bool(
            "showInv", "Show Player Inventory", true));
    ...
    @Override public String description() { return "Displays enchant page breakdown from inventory."; }
    ...
    public void render(DrawContext g, Theme theme, float x, float y) {
        ...
        EnchantPageResult inventory = EnchantPageScanner.scanPlayerInventory();
        ...
        if (result == null) {
            if (isEditor()) {
                float ph = ROW_H * 5 + 16f;
                size(WIDTH, ph);
                chrome(g, x, y, WIDTH, ph, 0f);
                txt(g, "ENCHANT PAGES", x + PAD, y + 2f, theme.accent());
                txt(g, "Inventory", x + WIDTH - Draw.textWidth("Inventory") - PAD, y + 2f, theme.faint());
                float ry = y + ROW_H + 4f;
                txt(g, "COMMON", x + PAD, ry, Color.rgb(255, 255, 255));
                ry += ROW_H;
                txt(g, "1% x3", x + PAD + 8f, ry, theme.text());
                ry += ROW_H;
                txt(g, "5% x1", x + PAD + 8f, ry, theme.text());
                ry += ROW_H;
                txt(g, "Total: 4  8%", x + PAD, ry + 4f, theme.accent());
                return;
            }
            size(WIDTH, ROW_H + 4);
            chrome(g, x, y, WIDTH, ROW_H + 4, 0f);
            txt(g, "No pages found", x + PAD, y + 2f, theme.faint());
            return;
        }
        ...
        // Header
        chrome(g, x, ry, WIDTH, totalH, 0f);

        String header = "ENCHANT PAGES";
        txt(g, header, x + PAD, ry + 2f, theme.accent());
        String sourceLabel = result.source().label();
        txt(g, sourceLabel, x + WIDTH - Draw.textWidth(sourceLabel) - PAD, ry + 2f, theme.faint());
        ry += ROW_H + 4f;
        ...
        for (RarityGroup group : result.groups()) {
            ...
            // Rarity header
            txt(g, group.rarity().toUpperCase(Locale.ROOT), x + PAD, ry + 1f, rarityColor);
            ry += ROW_H + 2f;

            // Percentage entries
            for (PctEntry entry : group.entries()) {
                String pctLabel = formatPct(entry.percentage()) + " x" + entry.quantity();
                txt(g, pctLabel, x + PAD + 8f, ry + 1f, theme.text());
                ry += ROW_H;
            }
        }
        ...
        txt(g, totalLine, x + PAD, ry, theme.accent());
    }
