...
public final class StrongholdPlayerWidget extends HudWidget {
    ...
    public void render(DrawContext g, Theme theme, float x, float y) {
        ...
        List<NearbyPlayer> raw = NearbyPlayerScanner.scan(radius.get());
        ...
        chrome(g, x, y, WIDTH, h, 4f);

        float ry = y;

        // Header
        Draw.text(g, "NEARBY PLAYERS", x + PAD, ry + 2f, theme.accent());
        ...
        for (NearbyPlayer p : players) {
            ...
            Draw.text(g, name, x + PAD, ry + 1f, nameColor);
            if (!infoStr.isEmpty()) {
                Draw.text(g, infoStr, x + WIDTH - infoW - PAD, ry + 1f, theme.dim());
            }
            ry += ROW_H;
        }
    }
