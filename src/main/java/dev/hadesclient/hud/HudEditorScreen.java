...
public final class HudEditorScreen extends UiScreen {
    ...
    protected void backdrop(Ctx ctx, DrawContext g) {
        ...
        for (HudWidget widget : hud.all()) {
            ...
            if (isSelected) {
                Draw.roundOutline(g, wx - 2, wy - 2, sw + 4, sh + 4, 0f, 1f,
                        theme.accent().alpha(0.9f));
            } else if (hovered) {
                Draw.roundOutline(g, wx - 1, wy - 1, sw + 2, sh + 2, 0f, 1f,
            ...
            if (hovered || isSelected) {
                ...
                Draw.roundRect(g, wx, wy + sh + 3, nw, 13, 0f, theme.panel().alpha(0.35f));
                Draw.text(g, name, wx + 4, wy + sh + 6, theme.text());

                // Scale badge
                String badge = String.format("%.2fx", widget.scale());
                float bw = Draw.textWidth(badge) + 8;
                Draw.roundRect(g, wx + sw - bw, wy + sh + 3, bw, 13, 0f, theme.panel().alpha(0.35f));
                ...
                Draw.roundRect(g, gearX, gearY, BTN_SIZE, BTN_SIZE, 0f,
                ...
                Draw.roundRect(g, xBtnX, xBtnY, BTN_SIZE, BTN_SIZE, 0f,
