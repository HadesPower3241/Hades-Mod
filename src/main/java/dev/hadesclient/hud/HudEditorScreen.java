...
public final class HudEditorScreen extends UiScreen {
    ...
    protected void backdrop(Ctx ctx, DrawContext g) {
        ...
        for (HudWidget widget : hud.all()) {
            ...
            if (active || hovered) {
                ...
                String cat = widget.category().label().toUpperCase();
                float cw = Draw.textWidth(cat) + 8;
                Draw.roundRect(g, wx, wy - 14, cw, 13, 3f, theme.panel().alpha(0.95f));
                Draw.text(g, cat, wx + 4, wy - 11, theme.dim().mix(theme.text(), 0.7f));
            }
        }
    ...
    private HudWidget widgetAt(double px, double py) {
        ...
        for (int i = all.size() - 1; i >= 0; i--) {
            ...
            if (!widget.enabled()) continue;            // disabled widgets aren't grabbable
            float wx = widget.resolveX(width);
            float wy = widget.resolveY(height);
