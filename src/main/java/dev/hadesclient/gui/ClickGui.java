...
public final class ClickGui extends UiScreen {
    ...
    private void buildWidgetSettings() {
        ...
        enableToggle.bounds(content.x() + w - 44f, y + 4f, 36f, 18f);
        ...
        for (int i = 0; i < bools.size(); i += 2) {
            ...
            tl.bounds(content.x(), y + 2f, 36f, 18f);
            content.add(tl);
            Element ll = labelElement(content.x() + 60f, y, halfW - 42f, left.name());
            content.add(ll);
            ...
            if (i + 1 < bools.size()) {
                ...
                tr.bounds(rx, y + 2f, 36f, 18f);
                content.add(tr);
                Element rl = labelElement(rx + 42f, y, halfW - 42f, right.name());
                content.add(rl);
            }
            y += 22f;
        }
    ...
    private void buildModuleSettings() {
        ...
        master.bounds(content.x() + w - 44f, y + 4f, 36f, 18f);
        content.add(master);
        y += 24f;
    ...
    private Element labelElement(float rx, float ry, float rw, String text) {
    Element label = new Element() {
        @Override
        protected void paint(Ctx ctx, DrawContext g) {
            Draw.text(g, text, x + 4f, y + 4f, ctx.theme().text());
        }
    };

    label.bounds(rx, ry, rw, 18f);
