...
public final class ClickGui extends UiScreen {
        ...
        for (int i = 0; i < TABS.length; i++) {
            ...
            Element pill = new Element() {
                @Override protected void paint(Ctx ctx, DrawContext g) {
                    ...
                    Draw.roundRect(g, x, y, w, h, 2f, fill);
                    Draw.roundOutline(g, x, y, w, h, 2f, 1f, edge);
        ...
        return new Element() {
            @Override protected void paint(Ctx ctx, DrawContext g) {
                ...
                if (active) {
                    ...
                    Draw.roundRect(g, x, y + 4, 2f, h - 8, 1.0f, theme.accent());
                } else if (lift > 0.01f) {
    ...
    private void buildWidgetSettings() {
        ...
        Element header = new Element() {
            @Override protected void paint(Ctx ctx, DrawContext g) {
                Theme theme = ctx.theme();
                Draw.roundRect(g, x, this.y, w, h, 2f, theme.raised().alpha(0.35f));
                Draw.roundOutline(g, x, this.y, w, h, 2f, 1f, theme.stroke().alpha(0.4f));
            ...
            Element row = new Element() {
                @Override protected void paint(Ctx ctx, DrawContext g) {
                    ...
                    for (Color swatch : List.of(value.accent(), value.panel(), value.raised(), value.ok(), value.bad())) {
                        Draw.roundRect(g, sx, this.y + h / 2f - 6, 12, 12, 1.0f, swatch);
                        sx += 15;
                    }
        ...
        for (int i = 0; i < cards.size(); i++) {
            ...
            Element element = new Element() {
                ...
                @Override protected void paint(Ctx ctx, DrawContext g) {
                    ...
                    float lift = hover.get();
                    float active = on.get();
                    Draw.roundRect(g, x, y, w, h, 2f, theme.raised().alpha(0.30f + 0.25f * lift));
                    ...
                    Draw.roundRect(g, x + 8, pillY, w - 16, 20f, 2f, pill);
    ...
    protected void backdrop(Ctx ctx, DrawContext g) {
        ...
        Draw.roundRect(g, winX, winY, winW, winH, 2f, theme.panel().alpha(0.35f * in));
        Draw.roundOutline(g, winX, winY, winW, winH, 2f, 1f, theme.stroke().alpha(0.5f * in));
