...
public final class ClickGui extends UiScreen {
    ...
    private static final float HEADER = 36f;
    private static final float SIDEBAR = 118f;
    private static final float CARD_H = 58f;
    ...
    private void buildSidebar() {
        ...
        if (TAB_MODS.equals(tab)) {
            ...
            allItem.bounds(winX + 8f, itemY, SIDEBAR - 16f, 20f);
            root().add(allItem);
            itemY += 22f;
            ...
            for (HudCategory hc : HudCategory.values()) {
                ...
                item.bounds(winX + 8f, itemY, SIDEBAR - 16f, 20f);
                root().add(item);
                itemY += 22f;
            }
        }
