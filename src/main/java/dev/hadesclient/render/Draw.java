...
public final class Draw {
    ...
    public static void text(DrawContext g, String text, float x, float y, Color color) {
        try {
            ...
            // Font error fallback — render with vanilla
            g.drawText(font(), text, Math.round(x), Math.round(y), color.argb(), false);
        }
    }
    ...
    public static void circle(DrawContext g, float cx, float cy, float radius, Color color) {
        ...
        for (int py = minY; py <= maxY; py++) {
            ...
            int x2 = (int) Math.floor(cx + dx);
            if (x2 >= x1) g.fill(x1, py, x2 + 1, py + 1, argb);
        }
    }

    /** Smooth circle outline. */
    public static void circleOutline(DrawContext g, float cx, float cy, float radius, float thickness, Color color) {
        if (color.invisible() || radius <= 0) return;
        int argb = color.argb();
        float outer = radius;
        float inner = radius - thickness;
        int minY = (int) Math.floor(cy - outer);
        int maxY = (int) Math.ceil(cy + outer);
        for (int py = minY; py <= maxY; py++) {
            float dy = py + 0.5f - cy;
            float outerDxSq = outer * outer - dy * dy;
            if (outerDxSq <= 0) continue;
            float outerDx = (float) Math.sqrt(outerDxSq);
            float innerDxSq = inner * inner - dy * dy;
            float innerDx = innerDxSq > 0 ? (float) Math.sqrt(innerDxSq) : 0;
            int ox1 = (int) Math.ceil(cx - outerDx);
            int ox2 = (int) Math.floor(cx + outerDx);
            int ix1 = (int) Math.floor(cx - innerDx);
            int ix2 = (int) Math.ceil(cx + innerDx);
            // Left arc
            if (ix1 > ox1) g.fill(ox1, py, ix1, py + 1, argb);
            // Right arc
            if (ox2 + 1 > ix2) g.fill(ix2, py, ox2 + 1, py + 1, argb);
        }
    }
