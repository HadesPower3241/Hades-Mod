 ...
 *
 * <p>Rounded corners are produced by scanning the corner arcs and emitting one
 * {@code fill} per row rather than by generating and tinting mask textures.
 * That trades a little antialiasing for a much smaller API surface — the whole
 * renderer needs only {@code fill}, {@code fillGradient}, {@code drawText} and
 * the scissor pair, all of which are stable across Minecraft versions. A
 * typical panel costs about a dozen extra fills, which the GUI batcher
 * swallows without noticing.</p>
 */
public final class Draw {
    ...
    public static void roundRect(DrawContext g, float x, float y, float w, float h, float radius, Color color) {
        ...
        int argb = color.argb();
        // Straight middle band in one call.
        g.fill(xi, yi + r, xi + wi, yi + hi - r, argb);
        // Corner rows, mirrored top and bottom.
    ...
    public static void roundOutline(DrawContext g, float x, float y, float w, float h,
        ...
        // Corner arcs: each row draws a short run wide enough to stay connected
        // to the row above it, so the curve never develops gaps.
        for (int row = 0; row < r; row++) {
            int inset = insetAt(r, row);
    ...
    /**
     * Soft drop shadow: concentric rounded rectangles fading outward. Draw it
     * before the panel it sits behind.
     */
    ...
    /** A full-screen dim, used behind menus. */
    ...
    /**
     * Horizontal inset of the corner arc at {@code row} rows in from the edge.
     * Derived from the circle equation, sampled at the row's centre.
     */
