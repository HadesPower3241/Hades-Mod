        ...
        if(!verified && current.fontId!=null && working) { verified=true;
            try { var tr=MinecraftClient.getInstance().textRenderer;
                if(tr!=null) { int v=tr.getWidth("TEST"), c=tr.getWidth(Text.literal("TEST").setStyle(cachedStyle));
                    if(v==c) HadesClient.LOG.warn("[HADES][FONT] '{}' same width as vanilla",current.displayName);
                    else HadesClient.LOG.info("[HADES][FONT] '{}' verified v={}px c={}px",current.displayName,v,c);
                }
            } catch(Throwable t) { HadesClient.LOG.error("[HADES][FONT] verify: {}",t.getMessage()); working=false; }
        }
