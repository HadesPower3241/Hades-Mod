    ...
    private static void draw(DrawContext g) {
        ...
        for (Entity guard : mc.world.getOtherEntities(mc.player, mc.player.getBoundingBox().expand(range))) {
            ...
            if (onScreen) {
                ...
                Draw.circle(g, targetX, targetY, DOT_SIZE, Color.rgb(40, 180, 255).alpha(a));
                ...
                Draw.roundRect(g, sx - PILL_W * 0.5f, sy - PILL_H * 0.5f, PILL_W, PILL_H, PILL_RAD, Color.rgb(12, 12, 18).alpha(0.78f * a));
                Draw.roundOutline(g, sx - PILL_W * 0.5f, sy - PILL_H * 0.5f, PILL_W, PILL_H, PILL_RAD, 1f, Color.rgb(40, 180, 255).alpha(0.45f * a));
                Draw.circle(g, sx - 20f, sy, 3.5f, Color.rgb(40, 180, 255).alpha(a));
                String lbl = dist >= 1000 ? String.format("%.1fk", dist / 1000) : String.format("%.0fm", dist);
                Draw.textCentered(g, lbl, sx + 5f, sy - 4f, Color.rgb(200, 225, 255).alpha(a));
            }
        }
    }
