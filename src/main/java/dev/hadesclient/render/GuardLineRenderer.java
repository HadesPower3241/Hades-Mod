...
public final class GuardLineRenderer {
    ...
    private static void render(WorldRenderContext context) {
        if (!GuardHighlighter.isEnabled() || !GuardHighlighter.isLineEnabled()) {
            return;
        }
        ...
        for (Entity entity : client.world.getEntities()) {
            if (!GuardHighlighter.isGuardEntity(entity) || client.player.squaredDistanceTo(entity) > rangeSquared) continue;
            ...
            float endY = (float)(guardPos.y + (double)entity.getHeight() * 0.5 - cameraPos.y);
            ...
            GuardLineRenderLayer.addVertex(consumer, matrix, startX, startY, startZ, red, green, blue, alpha, 2.0f);
            GuardLineRenderLayer.addVertex(consumer, matrix, endX, endY, endZ, red, green, blue, alpha, 2.0f);
        }
    }
}
