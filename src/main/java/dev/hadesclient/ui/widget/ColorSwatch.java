...
public class ColorSwatch extends Element {
    ...
    private static ColorSwatch activePicker = null; // only one open at a time
    ...
    public void tick(Ctx ctx, float dt) {
        super.tick(ctx, dt);
        // Expand height when picker is open to push down settings below
        if (pickerOpen) {
            h = 110f; // swatch row + picker popup
        } else {
            h = 22f; // just the swatch row
        }
    }
    ...
    protected boolean onClick(Ctx ctx, double mx, double my, int button) {
        ...
        if (pickerOpen) {
            ...
            pickerOpen = false; activePicker = null; return true;
        }
        // Close any other open picker first
        if (activePicker != null && activePicker != this) {
            activePicker.pickerOpen = false;
            activePicker.h = 22f;
        }
        pickerOpen = true;
        activePicker = this;
    ...
    protected boolean onPress(Ctx ctx, double mx, double my, int button) {
        return onDrag(ctx, mx, my, button);
    }
