...
public abstract class Setting {
    ...
    /** An RGB color setting stored as 0xRRGGBB int. */
    public static final class ColorVal extends Setting {
        private int rgb;
        public ColorVal(String id, String name, int defaultRgb) {
            super(id, name);
            this.rgb = defaultRgb & 0xFFFFFF;
        }
        public int get() { return rgb; }
        public void set(int rgb) { this.rgb = rgb & 0xFFFFFF; }
        public int red() { return (rgb >> 16) & 0xFF; }
        public int green() { return (rgb >> 8) & 0xFF; }
        public int blue() { return rgb & 0xFF; }
        public dev.hadesclient.theme.Color color() {
            return dev.hadesclient.theme.Color.rgb(red(), green(), blue());
        }
        public dev.hadesclient.theme.Color color(float alpha) {
            return color().alpha(alpha);
        }
        @Override public void save(com.google.gson.JsonObject json) {
            json.addProperty(id(), rgb);
        }
        @Override public void load(com.google.gson.JsonObject json) {
            if (json.has(id())) rgb = json.get(id()).getAsInt() & 0xFFFFFF;
        }
    }
}
